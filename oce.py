...
def stringtooperatorsequence(expression):
   """Converts a str to an operatorsequence object"""
   # Syntax of the str is rather loosely defined as:
   ...
   newsequences = []
   while (sequences.find("{") != -1):
      sequences = sequences[0:sequences.find("{")] + sequences[sequences.find("{")+1:]
   if (sequences[len(sequences)-1] != "}"):
      raise RuntimeError("Syntax error: the str must end with a normal ordered operator sequence")
      ...
      for index in sequence:
         ...
         if (len(index) == 1):
            ...
            if (((index >= "a") and (index <= "h")) or ((index >= "A") and (index <= "H"))):
               newsequence.append(Operator("particle",dagger,str.ascii_letters.index(index)+1,dummy))
            elif (((index >= "i") and (index <= "o")) or ((index >= "I") and (index <= "O"))):
               newsequence.append(Operator("hole",dagger,str.ascii_letters.index(index)+1,dummy))
            elif (((index >= "p") and (index <= "z")) or ((index >= "P") and (index <= "Z"))):
               newsequence.append(Operator("general",dagger,str.ascii_letters.index(index)+1,dummy))
         else:
            if (index[0] == "p"):
               newsequence.append(Operator("particle",dagger,int(index[1:]),dummy))
            elif (index[0] == "h"):
               newsequence.append(Operator("hole",dagger,int(index[1:]),dummy))
            elif (index[0] == "g"):
               newsequence.append(Operator("general",dagger,int(index[1:]),dummy))
            else:
   ...
   for element in breakdown:
      if (((element[0] >= 'a') and (element[0] <= 'z')) or \
         ...
         break
      numericalfactor = " ".join([numericalfactor, element])
   if (numericalfactor == ""):
   ...
   for element in breakdown:
      if ((element[0:3] == "SUM") or (element[0:3] == "sum") or (element[0:3] == "Sum")):
         join = 1
      if (join == 1):
         summationindexes = " ".join([summationindexes,element])
      elif (join == 2):
         remainder = " ".join([remainder,element])
      if ((join == 1) and (")" in element)):
         join = 2
   index = summationindexes.find("sum")
   if (index != -1):
   ...
   if (index != -1):
      summationindexes = summationindexes[0:index] + summationindexes[index+3:]
   index = summationindexes.find("(")
   if (index != -1):
      summationindexes = summationindexes[0:index] + summationindexes[index+1:]
   index = summationindexes.find(")")
   if (index != -1):
      summationindexes = summationindexes[0:index] + summationindexes[index+1:]
   summationindexes = summationindexes.split()
   summation = Summation([])
      ...
      if (len(index) == 1):
         ...
         if (((index >= "a") and (index <= "h")) or ((index >= "A") and (index <= "H"))):
            for indexinthelist in operatorlist:
               if ((indexinthelist.type == "particle") and (indexinthelist.index == str.ascii_letters.index(index)+1)):
                  summation.indexes.append(indexinthelist)
                  break
         elif (((index >= "i") and (index <= "o")) or ((index >= "I") and (index <= "O"))):
            for indexinthelist in operatorlist:
               if ((indexinthelist.type == "hole") and (indexinthelist.index == str.ascii_letters.index(index)+1)):
                  summation.indexes.append(indexinthelist)
                  break
         elif (((index >= "p") and (index <= "z")) or ((index >= "P") and (index <= "Z"))):
            for indexinthelist in operatorlist:
               if ((indexinthelist.type == "general") and (indexinthelist.index == str.ascii_letters.index(index)+1)):
                  summation.indexes.append(indexinthelist)
                  break
      else:
         if (index[len(index)-1:len(index)] == "+"):
            last = len(index) - 1
         else:
            last = len(index)
         if (index[0] == "p"):
            for indexinthelist in operatorlist:
               if ((indexinthelist.type == "particle") and (indexinthelist.index == int(index[1:last]))):
                  summation.indexes.append(indexinthelist)
                  break
         elif (index[0] == "h"):
            for indexinthelist in operatorlist:
               if ((indexinthelist.type == "hole") and (indexinthelist.index == int(index[1:last]))):
                  summation.indexes.append(indexinthelist)
                  break
         elif (index[0] == "g"):
            for indexinthelist in operatorlist:
               if ((indexinthelist.type == "general") and (indexinthelist.index == int(index[1:last]))):
                  summation.indexes.append(indexinthelist)
                  break
         else:
            print("syntax error")
   ...
   for tobeamplitude in tobeamplitudes:
      ...
      tobeamplitude = tobeamplitude.split("(")
...
def isidenticalto(one,two):
   ...
   if (one == []):
      for m in range(int(len(two)/2)):
         if (not two[m].isidenticalto(two[m+int(len(two)/2)])):
            return 0
      return 1
   if (two == []):
      for m in range(int(len(one)/2)):
         if (not one[m].isidenticalto(one[m+int(len(one)/2)])):
            return 0
      return 1
   if (len(one) != len(two)):
      return 0
   for n in range(int(len(one)/2)):
      found = 0
      for m in range(int(len(two)/2)):
         if ((one[n].isidenticalto(two[m])) and (one[n+int(len(one)/2)].isidenticalto(two[m+int(len(two)/2)]))):
            found = 1
      if (not found):
         return 0
   return 1
...
class Operator:
   ...
   def __init__(self,type="unknown",dagger="unknown",index=0,dummy=0):
      """Creates a second-quantized hole/particle/general creation/annihilation operator"""
      self.type = type
      self.dagger = dagger
      self.index = index
      self.dummy = dummy # if dummy = 1, this index does not exist (e.g. in IP-EOM), but just
                         # there to maintain the equal number of indices in super and sub.
                         # dummy only affects the very end stage of code generation.
   ...
   def show(self,showdummy=0):
      ...
      show = "".join( [self.type[0], repr(self.index)])
   ...
   def tex(self,showdummy=0):
      """Returns a LaTex form of output"""
      show = "".join( [self.type[0],"_{",repr(self.index),"}"])
      if (self.dummy):
         if (showdummy):
            if (self.dagger == "creation"):
              show = "".join( [show, "^{\dagger *}"])
            else:
              show = "".join( [show, "^*"])
         else:
            show = ""
      elif (self.dagger == "creation"):
         show = "".join( [show, "^{\dagger}"])
      return show

   def duplicate(self):
      """Returns a deepcopy of self"""
      duplicate = Operator(self.type,self.dagger,self.index,self.dummy)
      return duplicate
      
   def isidenticalto(self,another):
      """Checks if two second-quantized operators are identical"""
      if ((self.type == another.type) and (self.dagger == another.dagger) and (self.index == another.index) and (self.dummy == another.dummy)):
         return 1
      else:
         return 0
   ...
   def issimilarto(self,another):
      ...
      if ((self.type == another.type) and (self.dagger == another.dagger) and (self.dummy == another.dummy)):
         return 1
      else:
         return 0
   ...
   def showwithoutdagger(self,showdummy=0):
      """Returns a human-friendly str of the content"""
      show = "".join( [self.type[0], repr(self.index)])
      if (self.dummy):
         if (showdummy):
   ...
   def texwithoutdagger(self,showdummy=0):
      """Returns a human-friendly str of the content"""
      show = "".join([self.type[0],"_{",repr(self.index),"}"])
      if (self.dummy):
         if (showdummy):
   ...
   def isgreaterthan(self,another,operatorsequence):
      ...
      if ((not operatorsequence.summation.hastheindex(self)) and (not operatorsequence.summation.hastheindex(another))):
         ...
         if (selfconnectivity < anotherconnectivity):
            ...
            return 0
         elif (selfconnectivity == anotherconnectivity):
           if (self.index > another.index):
              return 1
           else:
              return 0
...
class Summation:
   ...
   def show(self,showdummy=0):
      ...
      for index in self.indexes:
         if (index.showwithoutdagger(showdummy)):
            show = " ".join([show, index.showwithoutdagger(showdummy)])
      show = " ".join([show,")"])
      return show
   ...
   def tex(self,showdummy=0):
      ...
      for index in self.indexes:
         if (show):
            ...
            show = "\\sum_{"
         show = " ".join([show,index.texwithoutdagger(showdummy)])
      show = " ".join([show,"}"])
      return show
...
#           return 1
   ...
   def canonicalize(self,operatorsequence):
      ...
      while (not done):
         ...
         for noperatora in range(int(len(another.indexes)/2)):
            for noperatorb in range(int(len(another.indexes)/2)):
      ...
      while (not done):
         ...
         # reorder sub indexes
         for noperatora in range(int(len(another.indexes)/2),len(another.indexes)):
            for noperatorb in range(int(len(another.indexes)/2),len(another.indexes)):
               if (noperatora >= noperatorb):
                  continue
               operatora = another.indexes[noperatora]
               operatorb = another.indexes[noperatorb]
...
#           show = " ".join([show,"/",repr(den)],"")
         if (coefficient >= 0.0):
            show = " ".join([show,"+",str(coefficient)])
         elif (coefficient < 0.0):
            show = " ".join([show,"-",str(-coefficient)])
         if (self.permutations[n]):
            show = " ".join([show,"* P("])
            for noperator in range(int(len(self.permutations[n])/2)):
               operator = self.permutations[n][noperator]
               if (operator.showwithoutdagger()):
                  show = " ".join([show,operator.showwithoutdagger()])
            show = " ".join([show,"=>"])
            for noperator in range(int(len(self.permutations[n])/2),len(self.permutations[n])):
               operator = self.permutations[n][noperator]
               if (operator.showwithoutdagger()):
                  show = " ".join([show,operator.showwithoutdagger()])
            show = " ".join([show,")"])
   ...
   def tex(self,showdummy=0):
      ...
      if (len(self.coefficients) > 1):
         ...
         for n in range(len(self.coefficients)):
            ...
            if (self.permutations[n]):
               show = " ".join([show,"P^{"])
               for nindex in range(int(len(self.permutations[n])/2),int(3*len(self.permutations[n])/4)):
                  index = self.permutations[n][nindex]
                  show = " ".join([show,index.texwithoutdagger(showdummy)])
               for nindex in range(int(len(self.permutations[n])/4),int(len(self.permutations[n])/2)):
                  index = self.permutations[n][nindex]
                  show = " ".join([show,index.texwithoutdagger(showdummy)])
               show = " ".join([show,"}_{"])
               for nindex in range(int(len(self.permutations[n])/4)):
                  index = self.permutations[n][nindex]
                  show = " ".join([show,index.texwithoutdagger(showdummy)])
               for nindex in range(int(3*len(self.permutations[n])/4),len(self.permutations[n])):
                  index = self.permutations[n][nindex]
                  show = " ".join([show,index.texwithoutdagger(showdummy)])
               show = " ".join([show,"}"])
            else:
...
class OperatorSequence:
   ...
   def show(self,showdummy=0):
      """Returns a human-friendly str of the content"""
      show = self.factor.show(showdummy)
      if (self.summation):
      ...
      if (self.sequence):
         ...
         for sequence in self.sequence:
            ...
            for operator in sequence:
               if (operator.show(showdummy)):
                  show = " ".join([show, operator.show(showdummy)])
            show = " ".join([show, "}"])
         show = " ".join([show, "|0>"])
      return show
   ...
   def tex(self,showdummy=0):
      """Returns a LaTeX str of the content"""
      show = self.factor.tex(showdummy)
#     if (self.summation):
#        if (len(self.summation.indexes) > 0):
#           show = " ".join([show, self.summation.tex()])
      for index in self.amplitudes:
         show = " ".join([show, index.tex(showdummy)])
      if (self.sequence):
         show = " ".join([show, "\\langle 0 |"])
         for sequence in self.sequence:
            show = " ".join([show, "\{"])
            for operator in sequence:
               show = " ".join([show, operator.tex(showdummy)])
            show = " ".join([show, "\}"])
         show = " ".join([show, "|0\\rangle"])
      return show
      ...
      for ncurly in range(len(self.sequence)):
         ...
         for noperator in range(len(self.sequence[ncurly])):
            ...
            if (leftmost.dagger == operator.dagger):
               continue
            elif (leftmost.dummy != operator.dummy):
               continue
...
class ListOperatorSequences:
   ...
   def show(self,showdummy=0):
      ...
      for operatorsequence in self.list:
         if (operatorsequence == "deleted"):
            ...
            show.append(operatorsequence.show(showdummy))
      return show
      ...
      for noperatorsequence in range(len(self.list)):
         ...
         if (operatorsequence != "deleted"):
            if (noperatorsequence == 0):
               ...
               show.append("".join(["&&",operatorsequence.tex(showdummy),"\\nonumber\\\\"]))
   ...
   def writetofile(self,filename):
      ...
      file = open(filename,"w")
      for operatorsequence in self.list:
         file.write(operatorsequence.show(1))
         file.write("\n")
